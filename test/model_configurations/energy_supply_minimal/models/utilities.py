...
def write_heat_demand_data(self, year, data):
    ...
    Columns are::
        ...
        day
        period
        eh_conn_num <- region id (1 to 29)
        heatload_res double precision,
        heatload_com double precision
    """
    conn = self.establish_connection()
    # Open a cursor to perform database operations
    cur = conn.cursor()
    ...
    sql = """INSERT INTO "HeatLoad_EH" (Year, Season, Day, Period, BusNumber, ElecLoad) VALUES (%s, %s, %s, %s, %s, %s)"""
    for row in elec_data:
        season, period = self.parse_season_period(row.interval)
        day = self._get_day(period)
        bus_number = int(bus_numbers[row.region])
