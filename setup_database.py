...
file_path = '/vagrant/config/database'


def drop_autogenerated_tables(prefix):
	"""Drop auto generated relations from database i.e. sector interventions
	"""
...
def down_migrations(working_path):
	...
	if isfile(join(working_path, file)):
		...
		subprocess.run(['psql', '-U', 'vagrant', '-d', 'nismod_smif', '-q', '-f', join(working_path, file)])
	else:
		return False
...
def up_migrations(working_path):
	...
	# run key file to add foreign keys between relations
	# run sql file silently
	subprocess.run(['psql', '-U', 'vagrant', '-d', 'nismod_smif', '-q', '-f', join(working_path, fky_file)])
...
def main():
	...
	config_dir = join(dirname(realpath(__file__)), 'config', 'database')

	# ask if user want the database to be populated/hydrated automatically
	auto_hydrate_db = input("Populate database with available data (y/n)? ")
	if auto_hydrate_db.lower() == 'y' or auto_hydrate_db.lower() == 'yes':
		print("You entered: %s. The database will be built and populated." % auto_hydrate_db)
		auto_hydrate_db = True
	elif auto_hydrate_db.lower() == 'n' or auto_hydrate_db.lower() == 'no':
		print("You entered: %s. The database will be built, but not populated." % auto_hydrate_db)
		auto_hydrate_db = False
	else:
		# if incorrect input provided, exit setup now and don't run anything
		print("Input incorrectly entered - you entered: %s. The database will not be setup or populated. Exiting setup." % auto_hydrate_db)
		exit()
		...
		with pysftp.Connection(host=host_address, username=username, password=password, cnopts=cnopts) as sftp:
			...
			with sftp.cd('project/database'):
				...
				with sftp.cd('nismod-db-vm'):
						...
						# confirm database hydration complete
						subprocess.run(['echo', 'Database hydration complete'])
main()